...
// === Slider Specific === 
let Minimum (value: float) = createProp Keys.Minimum value 
let Maximum (value: float) = createProp Keys.Maximum value 
let Value (value: float) = createProp Keys.Value value 
let OnValueChanged (f: ValueChangedEventArgs -> unit) = createProp Keys.Changed f 
// === Slider Specific === 
...
let Ref (viewRef: ViewRef<Slider>) = createProp Keys.Ref viewRef 
