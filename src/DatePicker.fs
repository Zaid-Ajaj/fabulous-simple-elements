...
// === DatePicker Specific ===
let Date (value: DateTime) = createProp "date" value 
let DateFormat (value: string) = createProp "format" value
let MinimumDate (minimumDate: DateTime) = createProp "minimumDate" minimumDate
let MaximumDate (maximumDate: DateTime) = createProp "maximumDate" maximumDate 
let OnDateChanged (f: DateChangedEventArgs -> unit) = createProp "dateSelected" f 
// === DatePicker Specific ===
...
let datePicker (props: IDatePickerProp list) = 
    let attributes = 
        props 
