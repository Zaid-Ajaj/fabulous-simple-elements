...
type IFlyoutPageProp =
    abstract name : string
    abstract value : obj

let internal createProp name value =
    { new IFlyoutPageProp with
        member x.name = name
        member x.value = value }

let Flyout (page: ViewElement) = createProp Keys.Flyout page
let Menu (page: ViewElement) = createProp Keys.Menu page
let IsPresented (condition: bool) = createProp Keys.IsPresented condition
let FlyoutBehaviour (behaviour: FlyoutBehavior) = createProp Keys.FlyoutBehaviour behaviour
...
let Ref (viewRef: ViewRef<FlyoutPage>) = createProp Keys.Ref viewRef
...
let inline flyoutPage(props:IFlyoutPageProp list) =
    let attributes =
        props
    ...
    View.FlyoutPage(?flyout=find Keys.Flyout,
        ?menu=find Keys.Menu,
        ?detail = find Keys.Detail,
        ?isPresented = find Keys.IsPresented,
        ?flyoutLayoutBehavior = find Keys.FlyoutBehaviour,
