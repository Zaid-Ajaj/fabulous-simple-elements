...
let Ref (viewRef: ViewRef<TabBar>) = createProp Keys.Ref viewRef

let OnCreated (f: TabBar -> unit) = createProp Keys.Created f

let inline tabBar(props: ITabBarProp list) : ViewElement =
    let attributes =
        props
